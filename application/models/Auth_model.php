...
class Auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		$validation_rules = array(
			...
			array('field' => 'register_confirm_password', 'label' => 'Confirm Password', 'rules' => 'required|matches[register_password]'),
                        array('field' => 'insert_group', 'label' => 'Group', 'rules' => 'required')
		);
		...
		if ($this->form_validation->run())
		{
			...
			$password = $this->input->post('register_password');
                        $user_group = $this->input->post('insert_group');
	...
	function update_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Redirect user.
			($response) ? redirect('auth_admin/update_account') : redirect('auth_admin/update_account');
		}
		else
		{		
			// Set validation errors.
	...
	function change_password()
	{
		...
		if ($this->form_validation->run())
		{
			...
			//$current_password = $this->input->post('current_password');
                        $identity = $this->input->post('userEmail');
                        $user_id = $this->input->post('userId');
                        $current_password = '';
			$new_password = $this->input->post('new_password');			
			
			// Note: Changing a password will delete all 'Remember me' database sessions for the user, except their current session.
			$response = $this->flexi_auth->change_password($identity, $current_password, $new_password, $user_id);
			...
			($response) ? redirect('auth_admin/manage_user_accounts') : redirect('auth_public/change_password');
		}
		else
		{		
			// Set validation errors.
