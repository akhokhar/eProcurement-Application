...
class Auth_admin_model extends CI_Model {
	...
	function get_user_accounts()
	{
		...
		// For this example, prevent any 'Master Admin' users (User group id of 3 defined in flexi auth config) being listed to non 'Master Admin' users.
   ...
   function manage_user_groups()
	...
	function update_privilege($privilege_id)
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Redirect user.
			redirect('auth_admin/update_privilege/'.$privilege_id);			
		}
	}
	...
	function update_user_privileges($user_id)
    {
		...
		$this->session->set_flashdata('message', $this->flexi_auth->get_messages());
                ...
                if($this->flexi_auth->get_user_id() == $user_id)
	...
	function update_group_privileges($group_id)
    {
		...
		$this->session->set_flashdata('message', $this->flexi_auth->get_messages());
                ...
                if($this->flexi_auth->get_user_group_id() == $group_id)
                    redirect('auth/logout');
                else
                    // Redirect user.
