...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	public function delete_user($user_id)
	{
		...
		//$this->db->delete($this->auth->tbl_user_account, array($this->auth->tbl_col_user_account['id'] => $user_id));
                ...
                $update_data = array($this->auth->tbl_col_user_account['active'] => 0);
                $this->db->where($this->auth->tbl_col_user_account['id'],$user_id);
                $this->db->update($this->auth->tbl_user_account,$update_data);
	...
	public function delete_group($sql_where)
  	{
		...
		//$this->db->delete($this->auth->tbl_user_group, $sql_where);*/
                $update_data = array($this->auth->tbl_col_user_group['active'] => 0);
                $this->db->where($this->auth->tbl_col_user_group['id'], $sql_where);
                $this->db->update($this->auth->tbl_user_group, $update_data);
	 ...
	 * @author Rob Hussey
	 */
	public function change_password($identity, $current_password, $new_password, $user_id)
	{		
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE)
	{
		...
		$sql_where = array($this->auth->primary_identity_col => $identity);
                $sql_or_where = array('uacc_username' => $identity);
                //$sql_where = array($this->auth->identity_cols => $identity);
